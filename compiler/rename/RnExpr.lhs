...
value bindings. This is done by checking that the name is external,
since all top-level names are external before the tidy-core phase. See
the Note about the NameSorts in Name.lhs.

\begin{code}
rnExpr e@(HsStatic expr)
  = do (expr',fvExpr) <- rnLExpr expr
       let isTopLevelValName n = isExternalName n && isValName n
       case nameSetToList $ filterNameSet (not . isTopLevelValName) fvExpr of
         [] -> return ()
         fvNonGlobal -> do
           addErr $ cat [ ptext $ sLit "Only top-level value bindings can be referred inside the static form"
                        , nest 2 $ ppr e
